...
int main(int argc, char** argv){
    ...
    // Random key generator for now, but should use shared secret key from ECDH
    if(!(RAND_bytes(shared_secret_k, sizeof(shared_secret_k)))){
    ...
    params[1] = OSSL_PARAM_construct_octet_string(OSSL_KDF_PARAM_KEY, shared_secret_k, sizeof(shared_secret_k));
    ...
    params[3] = OSSL_PARAM_construct_octet_string(OSSL_KDF_PARAM_SALT, salt, sizeof(salt));
    ...
    */
    if (EVP_KDF_derive(kdf_ctx, aes_key, sizeof(aes_key), params) <= 0) {
        printf("Error: Could not derive aes_key");
    }

    hexdump("Key:",shared_secret_k, sizeof(shared_secret_k));
    printf("Info: [%s]\n",info);
	hexdump("Salt", salt, sizeof(salt));
